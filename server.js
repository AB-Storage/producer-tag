...
function getHistory(req, res) {
  ...
  // Hide throwaway/auto-named repos (tool worktrees, scratch dirs) unless the
  // user explicitly configured one — keeps the list to real projects.
  const repos = [...names]
    .filter((n) => n && n !== '(unknown)' && (n in cfg.repos || !looksAutoGenerated(n)))
    .sort().map((name) => ({ name, enabled: willPlay(name) }));
...
// Looks like a throwaway repo a tool generated (docker-style "adjective-noun-hex"
// worktrees, "auto-*"/"agent-*" scratch dirs) rather than a real project.
function looksAutoGenerated(name) {
  return /^(auto|agent)-/i.test(name) || /^[a-z]+-[a-z]+-[0-9a-f]{6,}$/i.test(name);
}

// Find git repos under a folder (a dir containing a .git folder), bounded.
// Skips throwaway/auto-named repos. (Worktrees use a .git FILE, so the
// directory check below already ignores them.)
function findRepos(root, maxDepth) {
  ...
  while (stack.length && out.length < 300) {
    ...
    if (entries.some((e) => e.isDirectory() && e.name === '.git')) {
      ...
      if (!looksAutoGenerated(name)) out.push(name);
      continue;
    }
